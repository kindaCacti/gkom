...
constexpr unsigned int Roughness = 4;
} // namespace MeshAttrib
...
struct Mesh {
    ...
    Mesh(Mesh &&other) noexcept
        : VAO(other.VAO), VBO(other.VBO), EBO(other.EBO),
          ...
          hasRoughness(other.hasRoughness), minX(other.minX), maxX(other.maxX),
          minY(other.minY), maxY(other.maxY), minZ(other.minZ),
          maxZ(other.maxZ) {
        other.VAO = 0;
        other.VBO = 0;
        other.EBO = 0;
    ...
    Mesh(const std::vector<float> &vertices,
         ...
         bool hasColors, bool hasTexcoords, float minX, float maxX, float minY,
         float maxY, float minZ, float maxZ, bool hasRoughness = false)
        : indexCount(static_cast<unsigned int>(indices.size())),
          hasTexcoords(hasTexcoords), hasNormals(hasNormals),
          hasColors(hasColors), hasRoughness(hasRoughness), minX(minX),
        ...
        // Packed order: position, normal, color, texCoord, roughness
        ...
        if (hasRoughness) {
            glVertexAttribPointer(
                MeshAttrib::Roughness, 1, GL_FLOAT, GL_FALSE, strideBytes,
                (void *)(static_cast<std::intptr_t>(offsetFloats) *
                         sizeof(float)));
            glEnableVertexAttribArray(MeshAttrib::Roughness);
            offsetFloats += 1;
        }
    }
