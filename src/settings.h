...
inline void loadMeshesToSettings(Settings &settings, YAML::Node &yamlNode) {
    for (const auto &mesh : yamlNode) {
        ...
        if (mesh["color"].IsDefined())
            color = mesh["color"].as<glm::vec3>();
        if (mesh["position"].IsDefined())
            position = mesh["position"].as<glm::vec3>();
        if (mesh["rotation"].IsDefined())
            rotation = mesh["rotation"].as<glm::vec3>();
        if (mesh["scale"].IsDefined())
            scale = mesh["scale"].as<glm::vec3>();
        ...
        settings.meshes.push_back(MeshData{.path = path,
                                           .name = name,
                                           .colorOverride = color,
                                           .transform = transform});
    }
...
inline void loadTexturesToSettings(Settings &settings, YAML::Node &yamlNode) {
    for (const auto &texture : yamlNode) {
        ...
        settings.textures.push_back(TextureData{.path = path, .name = name});
    }
}
...
inline void loadDefaultsToSettings(Settings &settings, YAML::Node &yamlNode) {
    if (yamlNode["window"].IsDefined()) {
        if (yamlNode["window"]["width"].IsDefined())
            settings.windowWidth = yamlNode["window"]["width"].as<size_t>();
        if (yamlNode["window"]["height"].IsDefined())
            settings.windowHeight = yamlNode["window"]["height"].as<size_t>();
        if (yamlNode["window"]["title"].IsDefined())
            settings.title = yamlNode["window"]["title"].as<std::string>();
    }
    if (yamlNode["is_instanced"].IsDefined())
        settings.is_instanced = yamlNode["is_instanced"].as<bool>();
    if (yamlNode["is_benchmark"].IsDefined())
